...
import CasualFlapMapImageUrl from "./static/CasualFlatMap.png";
...
let wordMesh, axes, gridMesh,textMesh,birdMesh;
const mixers = [];
...
async function init() {
    ...
    earthNFTGeometry.translate(1, 0.2, 0);
        ...
        for (const result of results) {
            ...
            if (state == "tracked") {
                ...
                if (imageIndex == 0) {
                    ...
                    wordMesh.position.copy(markerWorldPosition);
                    wordMesh.rotation.setFromQuaternion(markerWorldQuaternion);
...
function createWordMesh(){
    ...
    wordMesh = new THREE.Group();
    // axes
    axes =new THREE.AxesHelper(10)
    wordMesh.add(axes);
    // grid
    gridMesh = new THREE.GridHelper(20, 20)
	//wordMesh.add(gridMesh);
    //GLTF
    ...
    loader.load( 'static/gltf/Flamingo.glb', function ( gltf ) {
        ...
        birdMesh.position.x = -3;
        birdMesh.position.y = 0.5;
        birdMesh.position.z = -3;
        birdMesh.renderOrder = 2
        //mesh.rotation.y = 0.5;

        //mesh.castShadow = true;
        //mesh.receiveShadow = true;

        wordMesh.add(birdMesh);
    ...
    font_loader.load( 'static/fonts/kenpixel.ttf', function ( json ) {
        ...
        wordMesh.add(textMesh)
    } );

    let wallMesh = new THREE.Group();
    wallMesh.add(createWallElement(new THREE.Vector3(-2, 0, -1.7), new THREE.Vector3(4, 4, 0.1)));
    wallMesh.add(createWallElement(new THREE.Vector3(1.5, 0, -2), new THREE.Vector3(0.1, 4, 4)));
    wallMesh.opacity = 0.01
    //wallMesh.visible = false

    wordMesh.add(wallMesh);
    scene.add(wordMesh);
...
function setupGeometry() {
    ...
    navigationArea.add(createWallElement(new THREE.Vector3(0, 0, -1), new THREE.Vector3(0, 0, 0), new THREE.Vector3(0.0625, 3, 1.578)));
...
function createWallElement(position, scale) {
